  ...
  if (supportsSetCodecPreferences) {
    ...
    if (preferredCodec.value !== '') {
      ...
      const selectedCodecIndex = codecs.findIndex(
        c => c.mimeType === mimeType && c.clockRate === parseInt(clockRate, 10) && c.sdpFmtpLine === sdpFmtpLine,
      );
      ...
      const transceiver = pc1
        .getTransceivers()
        .find(t => t.sender && t.sender.track === localStream.getAudioTracks()[0]);
    ...
    res.forEach(report => {
      ...
      if (report.type === 'outbound-rtp') {
        ...
        if (lastResult && lastResult.has(report.id)) {
          ...
          packetSeries.addPoint(now, (1000 * (packets - lastResult.get(report.id).packetsSent)) / deltaT);
